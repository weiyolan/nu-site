...
import { type PageId, type PageSlug, pageIdSchema, pages, pageSlugSchema } from "@/lib/db/schema/pages";
...
export const getPageBySlugWithPageLinks = async (slug: PageSlug) => {
  ...
  const rows = await db.select({ page: pages, pageLink: pageLinks }).from(pages).where(eq(pages.slug, pageSlug)).leftJoin(pageLinks, eq(pages.id, pageLinks.pageId));
  if (rows.length === 0) return {};
  const p = rows[0].page;
  const pp = rows.filter((r) => r.pageLink !== null).map((p) => p.pageLink) as CompletePageLink[];
