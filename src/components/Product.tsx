...
export interface ProductProps extends React.HTMLAttributes<HTMLDivElement> {
  product: {
    ...
    price: number;
    rating: number;
    type: "title" | "product" | "article" | "shopTitle";
    color?: "bg-nu-yellow" | "bg-nu-green" | "bg-nu-blue" | "bg-nu-peach" | "bg-nu-purple";
  };
  locale: string;
}

export default function Product({ product: { slug, title, type, description, price, rating, color, images }, locale, className, ...props }: ProductProps) {
  return (
    <div
      className={cn(
        `flex relative flex-col group w-full gap-2 sm:gap-3 lg:gap-4  ${type === "title" || "shopTitle" ? " justify-center text-center py-4 px-2 sm:px-3 lg:px-4" : " items-start"}`,
        className
      )}
      {...props}>
        ...
        <div className="w-full">
          <AspectRatio className="relative">
            ...
            <Image
              ...
              sizes="25vw"
              placeholder="blur"
              blurDataURL={images[1].asset.metadata.lqip}
              className="object-cover opacity-0 group-hover:opacity-100 blur group-hover:blur-0  transition-all duration-300  "
            />
          </AspectRatio>
        </div>
      )}
      {(type === "title" || "shopTitle") && <div className={`absolute z-0 top-0 left-0 right-0 bottom-0 ${color}`} />}
      ...
      {type === "article" && (
        <Button variant="link" asChild size="sm" className=" relative items-center -mt-2 -ml-2 group/button text-base">
          <Link className="" href={`/shop#${slug}`}>
            Lire article <ChevronRight className="size-4 group-hover/button:translate-x-1 transition-transform   mt-1" />
          </Link>
        </Button>
      )}
    </div>
