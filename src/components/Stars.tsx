...
export default function Stars({ className, rating, options, ...props }: StarsProps) {
  ...
  return (
    <div className={cn("flex gap-1 justify-center items-center ", options?.dark ? "text-nu-black" : "text-nu-yellow-dark", className)} {...props}>
      <svg className={cn(" w-24 lg:w-[120px] h-auto", options?.large && "w-[180px] ")} viewBox="0 0 120 19">
        <mask id={`starMask${uniqueId}`} style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x="0" y="0" width="100%" height="19">
          <rect width={4 + Math.floor(rating * 5) * (8 + 16) + rest * 16} x="0" y="0" height="20" fill="#ffffff" />
        </mask>
        <g mask={`url(#starMask${uniqueId})`}>
          <path
            className={(options?.dark ? "fill-nu-black" : "fill-nu-yellow-dark") + " stroke-transparent"}
        ...
        <path
          className={(options?.dark ? "stroke-nu-black" : "stroke-nu-yellow-dark") + " fill-transparent"}
          d="M12 2L14.472 7.008L20 7.816L16 11.712L16.944 17.216L12 14.616L7.056 17.216L8 11.712L4 7.816L9.528 7.008L12 2Z"
          strokeWidth="2"
          strokeLinecap="round"
      ...
      <Typography variant={"p"} className={cn("w-fit pl-0.5 text-xs md:text-sm font-bold pt-1 md:pt-0.5 leading-4", options?.large && "text-xl ")}>
