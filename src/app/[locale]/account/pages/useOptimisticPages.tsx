...
export const useOptimisticPages = (pages: CompletePage[]) => {
  const [optimisticPages, addOptimisticPage] = useOptimistic(pages, (currentState: CompletePage[], action: OptimisticAction<Page>): CompletePage[] => {
    const { data } = action;
    ...
    switch (action.action) {
      case "create":
        return currentState.length === 0 ? [optimisticPage] : [...currentState, optimisticPage];
      case "update":
        return currentState.map((item) => (item.id === data.id ? { ...item, ...optimisticPage } : item));
      case "delete":
