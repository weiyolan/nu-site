...
export function useValidatedForm<Entity>(insertEntityZodSchema: ZodSchema) {
  ...
  const hasErrors = errors !== null && Object.values(errors).some((error) => error !== undefined);
  ...
  const handleChange = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (target instanceof HTMLInputElement || target instanceof HTMLSelectElement || target instanceof HTMLTextAreaElement) {
      if (!(target instanceof HTMLInputElement && target.type === "submit")) {
        ...
        const fieldError = result.success ? undefined : result.error.flatten().fieldErrors[field];
